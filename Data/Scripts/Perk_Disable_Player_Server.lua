...
evts[#evts + 1] = Events.ConnectForPlayer("perk_disable_player", function(player)
...
evts[#evts + 1] = Events.ConnectForPlayer("perk_enable_player", function(player)
	if(players[player.id] ~= nil) then
		...
		-- Just in case as a backup enable movement.  Should never get to this point.
